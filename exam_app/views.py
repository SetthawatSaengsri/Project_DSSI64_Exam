...
def index_view(request):
    """หน้าแรกของระบบ - ไม่มีการสมัคร แค่แสดง landing page"""
    return render(request, 'app/index.html')

def login_user(request):
    """เข้าสู่ระบบ - ตรวจสอบอีเมล/รหัสผ่าน แล้วพาไป Dashboard ตามบทบาท"""
    if request.method == 'POST':
        ...
        # Redirect ตามบทบาท (ตรวจสอบตามลำดับความสำคัญ)
        if user.is_superuser:  # Admin/Staff (รวมกัน)
            messages.success(request, f'ยินดีต้อนรับ ผู้ดูแลระบบ {user.get_full_name()}')
            return redirect('dashboard_staff')
        elif user.is_teacher:
            messages.success(request, f'ยินดีต้อนรับ ครู {user.get_full_name()}')
            return redirect('dashboard_teacher')
        elif user.is_student:
            messages.success(request, f'ยินดีต้อนรับ นักเรียน {user.get_full_name()}')
            return redirect('dashboard_student')
        else:
            # กรณีที่ไม่มีบทบาทที่ชัดเจน
            messages.warning(request, 'ไม่พบบทบาทของผู้ใช้ กรุณาติดต่อผู้ดูแลระบบ')
            logout(request)
            return redirect('index_view')
...
def invigilated_subjects(request):
    """หน้ารายการวิชาที่ครูคุมสอบ - พร้อมสถานะการเช็คชื่อของครูตัวเอง"""
    if not request.user.is_teacher:
        return HttpResponseForbidden("คุณไม่มีสิทธิ์เข้าถึงส่วนนี้")
    ...
    # วิชาที่ครูคุมสอบ (ทั้งหลักและสำรอง)
    ...
    # นิยามเงื่อนไขสถานะตามวัน-เวลา
    upcoming_q = Q(exam_date__gt=today) | (Q(exam_date=today) & Q(start_time__gt=now_time))
    ongoing_q  = Q(exam_date=today) & Q(start_time__lte=now_time) & Q(end_time__gt=now_time)
    completed_q = Q(exam_date__lt=today) | (Q(exam_date=today) & Q(end_time__lte=now_time))

    # นับสถิติโดยอิงจากตัวกรองปี/เทอม (เพื่อแสดง card summary)
    base_qs = subjects
    upcoming_count  = base_qs.filter(upcoming_q).count()
    ongoing_count   = base_qs.filter(ongoing_q).count()
    completed_count = base_qs.filter(completed_q).count()

    # นับจำนวนวิชาที่เป็นครูหลักและครูสำรอง
    primary_count = base_qs.filter(invigilator=teacher).count()
    secondary_count = base_qs.filter(secondary_invigilator=teacher).count()

    # คำนวณจำนวนนักเรียนทั้งหมด
    total_students = sum(s.get_student_count() for s in base_qs)
    
    # คำนวณค่าเฉลี่ยนักเรียนต่อวิชา
    avg_students_per_subject = total_students / base_qs.count() if base_qs.count() > 0 else 0
    
    # คำนวณเปอร์เซ็นต์วิชาที่สอบเสร็จ
    completed_percentage = (completed_count / base_qs.count() * 100) if base_qs.count() > 0 else 0

    # ฟิลเตอร์สถานะ (ถ้ามีเลือกจากดรอปดาวน์)
    status_filter = (request.GET.get('status') or '').strip()
    if status_filter == 'upcoming':
        subjects = subjects.filter(upcoming_q)
    elif status_filter == 'ongoing':
        subjects = subjects.filter(ongoing_q)
    elif status_filter == 'completed':
        subjects = subjects.filter(completed_q)

    # ===== แก้ไขตรงนี้: เพิ่มสถานะการเช็คชื่อของครูตัวเอง =====
    for subject in subjects:
        # ตรวจสอบว่าครูคนนี้เป็นครูหลักหรือครูสำรอง
        is_primary = subject.invigilator == teacher
        is_secondary = subject.secondary_invigilator == teacher
        
        # กำหนดบทบาทและสถานะการเช็คชื่อของครูตัวเอง
        if is_primary:
            subject.my_role = 'primary'
            subject.my_check_in_status = 'checked_in' if subject.invigilator_checkin else 'not_checked_in'
            subject.my_check_in_time = subject.invigilator_checkin_time
        elif is_secondary:
            subject.my_role = 'secondary'
            subject.my_check_in_status = 'checked_in' if subject.secondary_invigilator_checkin else 'not_checked_in'
            subject.my_check_in_time = subject.secondary_invigilator_checkin_time
        else:
            # ไม่ควรเกิดขึ้น เพราะ query กรองแล้ว
            subject.my_role = 'unknown'
            subject.my_check_in_status = 'unknown'
            subject.my_check_in_time = None

    # ตัวเลือกปีที่ครูคุมสอบ (ไว้แสดงในดรอปดาวน์)
    years = ExamSubject.objects.filter(
        Q(invigilator=teacher) | Q(secondary_invigilator=teacher)
    ).values_list('academic_year', flat=True).distinct().order_by('-academic_year')
    
    # ตัวเลือกระดับชั้นที่ครูคุมสอบ
    classes = StudentProfile.objects.filter(
        Q(exam_subjects__invigilator=teacher) | 
        Q(exam_subjects__secondary_invigilator=teacher)
    ...
    return render(request, 'app/teacher/invigilated_subjects.html', {
        ...
        'avg_students_per_subject': avg_students_per_subject,
...
def teacher_manual_checkin(request):
    """
    AJAX endpoint สำหรับให้ครูเช็คชื่อตัวเองว่าเข้าคุมสอบแล้ว
    
    Parameters:
        - subject_id: ID ของวิชาสอบ
    ...
    try:
        ...
        try:
            ...
            return JsonResponse({
                'success': False,
                'message': 'ไม่พบวิชาสอบที่ระบุ'
            }, status=404)
        
        # ตรวจสอบว่าครูคนนี้เป็นครูคุมสอบของวิชานี้หรือไม่
        is_primary = subject.invigilator == teacher_profile
        is_secondary = subject.secondary_invigilator == teacher_profile
        
        if not (is_primary or is_secondary):
            return JsonResponse({
                'success': False,
                'message': 'คุณไม่ได้เป็นครูคุมสอบของวิชานี้'
            }, status=403)
        ...
        if is_primary:
            ...
            if subject.invigilator_checkin:
                return JsonResponse({
                    'success': False,
                    'message': 'คุณได้เช็คชื่อไปแล้ว',
                    'already_checked': True,
                    'checkin_time': subject.invigilator_checkin_time.strftime('%H:%M:%S') if subject.invigilator_checkin_time else ''
                })
            ...
            return JsonResponse({
                'success': True,
                'message': 'เช็คชื่อเข้าคุมสอบสำเร็จ (ครูหลัก)',
                'role': 'primary',
            ...
            if subject.secondary_invigilator_checkin:
                return JsonResponse({
                    ...
                    'message': 'คุณได้เช็คชื่อไปแล้ว',
                    'already_checked': True,
                    'checkin_time': subject.secondary_invigilator_checkin_time.strftime('%H:%M:%S') if subject.secondary_invigilator_checkin_time else ''
                })
            ...
            return JsonResponse({
                'success': True,
                'message': 'เช็คชื่อเข้าคุมสอบสำเร็จ (ครูสำรอง)',
                'role': 'secondary',
                'checkin_time': now.strftime('%H:%M:%S'),
                'checkin_date': now.strftime('%d/%m/%Y')
            })
        ...
        print(f"Teacher check-in error: {str(e)}")
        import traceback
        traceback.print_exc()
        return JsonResponse({
            'success': False,
...
@login_required
@require_http_methods(["POST"])
def teacher_checkout(request):
    """
    AJAX endpoint สำหรับให้ครูยกเลิกการเช็คชื่อ (กรณีเช็คผิด)
    ...
    try:
        ...
        if is_primary:
            ...
            return JsonResponse({
                'success': True,
                'message': 'ยกเลิกการเช็คชื่อสำเร็จ (ครูหลัก)',
                'role': 'primary'
            })
            ...
            return JsonResponse({
                'success': True,
                'message': 'ยกเลิกการเช็คชื่อสำเร็จ (ครูสำรอง)',
                'role': 'secondary'
            })
        
    except Exception as e:
        print(f"Teacher check-out error: {str(e)}")
        return JsonResponse({
            'success': False,
            'message': f'เกิดข้อผิดพลาด: {str(e)}'
        }, status=500)
...
def teacher_dashboard_data_api(request):
    ...
    try:
        ...
        print(f"Teacher Dashboard API called with: year={year}, term={term}, class={class_filter}")
        
        # สร้าง queryset สำหรับ subjects ที่ครูคนนี้คุมสอบ
        subjects_query = ExamSubject.objects.filter(
            Q(invigilator=teacher_profile) | 
            Q(secondary_invigilator=teacher_profile)
        )
        ...
        if term:
            subjects_query = subjects_query.filter(term=term)
        if class_filter:
            # กรองตามระดับชั้นของนักเรียนในวิชา
            subjects_query = subjects_query.filter(
                students__student_class=class_filter
            ).distinct()
        ...
        for subject in subjects_query:
            if class_filter:
                # นับเฉพาะนักเรียนในระดับชั้นที่เลือก
                total_students += subject.students.filter(student_class=class_filter).count()
            else:
                # นับนักเรียนทั้งหมด
                total_students += subject.students.count()
...
@login_required
def get_teacher_class_list_api(request):
    """API สำหรับดึงรายการระดับชั้นที่ครูคุมสอบ"""
    if not request.user.is_teacher:
        return JsonResponse({'error': 'ไม่มีสิทธิ์เข้าถึง'}, status=403)
    ...
    try:
        ...
        # ดึงรายการระดับชั้นจากนักเรียนที่อยู่ในวิชาที่ครูคุมสอบ
        classes = StudentProfile.objects.filter(
            exam_subjects__invigilator=teacher_profile
        ).values_list('student_class', flat=True).distinct().order_by('student_class')
        
        # เพิ่มจากวิชาที่เป็นผู้คุมสอบสำรอง
        secondary_classes = StudentProfile.objects.filter(
            exam_subjects__secondary_invigilator=teacher_profile
        ).values_list('student_class', flat=True).distinct().order_by('student_class')
        
        # รวมและเอาค่าที่ไม่ซ้ำ
        all_classes = set(list(classes) + list(secondary_classes))
        all_classes = [cls for cls in all_classes if cls]  # กรองค่า None ออก
        all_classes.sort()
        ...
        print(f"Get teacher class list error: {str(e)}")
        return JsonResponse({
            'success': False,
            'error': f'เกิดข้อผิดพลาด: {str(e)}'
        }, status=500)
...
@login_required
def get_teacher_academic_years_api(request):
    """ดึงปีการศึกษาเฉพาะที่ครูคุมสอบ"""
    teacher_profile = request.user.teacher_profile
...
# Dashboard นักเรียน - อัปเดตใหม่
@login_required
def dashboard_student(request):
    """Dashboard นักเรียนแบบเต็มรูปแบบ"""
    ...
    ).select_related('room', 'room__building').order_by('exam_date', 'start_time')
    
    # การเข้าสอบล่าสุด
    my_attendance = Attendance.objects.filter(
        student=student_profile
    ).select_related('subject', 'subject__room').order_by('-checkin_time')[:10]
...
@login_required  
def student_dashboard_data_api(request):
    """API สำหรับดึงข้อมูลแดชบอร์ดนักเรียน - รองรับทุกสถานะ"""
    if not request.user.is_student:
        return JsonResponse({'error': 'ไม่มีสิทธิ์เข้าถึง'}, status=403)
    ...
    try:
        ...
        print(f"Student Dashboard API called with: year={year}, term={term}")
        
        # สร้าง queryset สำหรับ subjects ของนักเรียนคนนี้
        subjects_query = ExamSubject.objects.filter(students=student_profile)
        ...
        # นับวิชาที่กำลังจะสอบ (จากวันนี้เป็นต้นไป)
        today = timezone.now().date()
        upcoming_subjects = subjects_query.filter(exam_date__gte=today).count()
        
        # สถิติการเข้าสอบ - รวมทุกสถานะ
        attendance_query = Attendance.objects.filter(
            student=student_profile,
            subject__in=subjects_query
        )
        ...
        # คำนวณอัตราเข้าสอบ
        total_possible = total_subjects
        attendance_rate = (attended_count / total_possible * 100) if total_possible > 0 else 0
        ...
        chart_data = {
            'labels': ['เข้าสอบตรงเวลา', 'เข้าสอบสาย', 'ขาดสอบ', 'ลาป่วย/ลากิจ', 'ทุจริต'],
            'datasets': [{
                'label': 'จำนวนครั้ง',
                'data': list(attendance_stats.values()),
                'backgroundColor': [
                    ...
                    'rgba(234, 179, 8, 0.8)',   # เหลือง - สาย  
                    'rgba(239, 68, 68, 0.8)',   # แดง - ขาด
                    'rgba(6, 182, 212, 0.8)',   # ฟ้า - ลา
                    'rgba(249, 115, 22, 0.8)'   # ส้ม - ทุจริต
                ],
                'borderColor': 'rgba(34, 197, 94, 0.8)',
                'borderWidth': 2,
                'fill': False
        ...
        # ดึงวิชาที่กำลังจะสอบ (5 วิชาแรก)
        upcoming_exams = subjects_query.filter(
        ...
        for exam in upcoming_exams:
            upcoming_exams_data.append({
                ...
                'exam_date_display': exam.exam_date.strftime('%d/%m/%Y') if exam.exam_date else '',
                'start_time': exam.start_time.strftime('%H:%M') if exam.start_time else '',
                'end_time': exam.end_time.strftime('%H:%M') if exam.end_time else '',
                'room': exam.room.name if exam.room else None
            })
        
        # ดึงประวัติการเข้าสอบล่าสุด (5 รายการ)
        recent_attendance = attendance_query.order_by('-checkin_time')[:5]
        ...
        for att in recent_attendance:
            ...
            recent_attendance_data.append({
                ...
                'status': att.status,
                'checkin_time_display': checkin_time.strftime('%d/%m/%Y %H:%M น.') if checkin_time else ''
            })
        ...
        print(f"Returning student data: {response_data}")
        return JsonResponse(response_data)
        
    except Exception as e:
        print(f"Student Dashboard API error: {str(e)}")
        import traceback
        traceback.print_exc()
        return JsonResponse({
            'success': False,
...
@login_required
def get_student_academic_years_api(request):
    """API สำหรับดึงรายการปีการศึกษาที่นักเรียนมีวิชาสอบ"""
    if not request.user.is_student:
        return JsonResponse({'error': 'ไม่มีสิทธิ์เข้าถึง'}, status=403)
    ...
    try:
        ...
        # ดึงปีการศึกษาจากวิชาที่นักเรียนลงทะเบียน
        years = ExamSubject.objects.filter(
            students=student_profile
        ).values_list('academic_year', flat=True).distinct().order_by('-academic_year')
        ...
        print(f"Get student academic years error: {str(e)}")
        # ส่งปีเริ่มต้นหากเกิดข้อผิดพลาด
...
@login_required
def student_exam_detail_api(request, exam_id):
    """API สำหรับดึงรายละเอียดวิชาสอบ"""
    if not request.user.is_student:
        return JsonResponse({'error': 'ไม่มีสิทธิ์เข้าถึง'}, status=403)
    ...
    try:
        ...
        exam = ExamSubject.objects.filter(
            id=exam_id,
            students=student_profile
        ).select_related('room', 'room__building', 'invigilator', 'secondary_invigilator').first()
        
        if not exam:
            return JsonResponse({
                'success': False,
                'error': 'ไม่พบวิชาสอบนี้หรือคุณไม่มีสิทธิ์เข้าถึง'
            }, status=404)
        
        # ดึงข้อมูลการเข้าสอบ (ถ้ามี)
        ...
        exam_data = {
            ...
            'subject_name': exam.subject_name,
            'exam_date': exam.exam_date.strftime('%d/%m/%Y') if exam.exam_date else None,
            ...
            'room': {
                'name': exam.room.name,
                'building': exam.room.building.name,
                'capacity': exam.room.capacity
            } if exam.room else None,
            'invigilator': exam.invigilator.user.get_full_name() if exam.invigilator else None,
            'secondary_invigilator': exam.secondary_invigilator.user.get_full_name() if exam.secondary_invigilator else None,
            'academic_year': exam.academic_year,
            'term': exam.term,
            'attendance': {
                'status': attendance.status,
                'checkin_time': attendance.checkin_time.strftime('%d/%m/%Y %H:%M น.') if attendance.checkin_time else None,
        ...
        print(f"Get exam detail error: {str(e)}")
        return JsonResponse({
            'success': False,
            'error': f'เกิดข้อผิดพลาด: {str(e)}'
        }, status=500)
...
def student_attendance_history_api(request):
    """API สำหรับดึงประวัติการเข้าสอบทั้งหมด"""
    if not request.user.is_student:
        return JsonResponse({'error': 'ไม่มีสิทธิ์เข้าถึง'}, status=403)
    ...
    try:
        ...
        # ดึงประวัติการเข้าสอบทั้งหมด
        attendance_list = Attendance.objects.filter(
            student=student_profile
        ).select_related('subject', 'subject__room').order_by('-checkin_time')
        
        # กรองตามเงื่อนไข (ถ้ามี)
        year = request.GET.get('year')
        term = request.GET.get('term')
        status = request.GET.get('status')
        
        if year:
            attendance_list = attendance_list.filter(subject__academic_year=year)
        if term:
            attendance_list = attendance_list.filter(subject__term=term)
        if status:
        ...
        for att in attendance_list:
            ...
            attendance_data.append({
                ...
                'status': att.status,
                'checkin_time': checkin_time.strftime('%d/%m/%Y %H:%M น.') if checkin_time else None,
                'seat_number': att.seat_number,
                'room': att.subject.room.name if att.subject.room else None,
                'exam_date': att.subject.exam_date.strftime('%d/%m/%Y') if att.subject.exam_date else None
            })
...
def student_exam_schedule(request):
    """หน้าตารางสอบของนักเรียน - แบบเต็มรูปแบบ"""
    if not request.user.is_student:
        return HttpResponseForbidden("คุณไม่มีสิทธิ์เข้าถึงส่วนนี้")
    ...
    upcoming_count = subjects.filter(upcoming_q).count()
    ongoing_count = subjects.filter(ongoing_q).count()
    completed_count = subjects.filter(completed_q).count()
    ...
    attendance_rate = round((attendance_count / total_subjects * 100), 1) if total_subjects > 0 else 0
    ...
    # ดึงข้อมูลการเข้าสอบของนักเรียนสำหรับแต่ละวิชา
    # เพิ่ม method ใน queryset เพื่อให้เข้าถึงได้ใน template
    attendance_dict = {}
    for att in Attendance.objects.filter(student=student, subject__in=subjects):
        attendance_dict[att.subject_id] = att

    # เพิ่มข้อมูลการเข้าสอบให้กับแต่ละวิชา
    for subject in subjects:
        subject.student_attendance = attendance_dict.get(subject.id, None)

    # ตัวเลือกปีที่นักเรียนมีวิชาสอบ
    years = ExamSubject.objects.filter(
        students=student
    ).values_list('academic_year', flat=True).distinct().order_by('-academic_year')

    return render(request, 'app/student/student_exam_schedule.html', {
        'subjects': subjects,
        'years': years,
        'year_filter': year_filter,
        'term_filter': term_filter,
...
def find_available_teachers(exam_date, start_time, end_time, min_count=1):
    """หาครูที่ว่างในช่วงเวลาที่กำหนด """
    try:
        from datetime import datetime
...
def exam_attendance(request, pk):
    ...
    is_invigilator = (
        is_teacher and (
            getattr(subject, "invigilator", None) == request.user.teacher_profile or
            getattr(subject, "secondary_invigilator", None) == request.user.teacher_profile
        )
    )
    can_edit = request.user.is_staff or is_invigilator
    if not can_edit:
        return HttpResponseForbidden("คุณไม่มีสิทธิ์เข้าถึงหน้านี้")

    # ===== ตรวจสอบว่าสอบจบแล้วหรือยัง =====
    if hasattr(subject, "is_finished") and callable(subject.is_finished):
        exam_completed = subject.is_finished()
    else:
        now = timezone.now()
        exam_completed = (
            subject.exam_date < now.date() or
            (subject.exam_date == now.date() and subject.end_time < now.time())
        )

    # ===== Finalize 'absent' อัตโนมัติเมื่อหมดเวลาสอบ =====
    if exam_completed:
        # กันซ้ำด้วย cache lock กรณีมีหลาย request เข้ามาพร้อมกัน
        lock_key = f"finalize_absent:subject:{subject.id}"
        got_lock = cache.add(lock_key, "1", timeout=60)  # ล็อก 60 วินาที
        if got_lock:
            try:
                with transaction.atomic():
                    # เวลา "สิ้นสุดสอบ" (ใช้บันทึกใน checkin_time ของ absent เพื่อ audit)
                    exam_end_dt = timezone.make_aware(
                        datetime.combine(subject.exam_date, subject.end_time)
                    )

                    # หา student ที่ยังไม่มี Attendance ของรายวิชานี้
                    existing_ids = set(
                        Attendance.objects.filter(subject=subject)
                        .values_list("student_id", flat=True)
                    )
                    missing_ids = list(
                        subject.students.exclude(id__in=existing_ids)
                        .values_list("id", flat=True)
                    )

                    if missing_ids:
                        # พยายามใส่ recorded_by ถ้ามี field นี้ในโมเดล
                        rows = []
                        for sid in missing_ids:
                            kwargs = dict(
                                ...
                                subject=subject,
                                status="absent",
                                checkin_time=exam_end_dt,
                            )
                            # เผื่อบางโปรเจกต์ไม่มีฟิลด์ recorded_by
                            if "recorded_by" in [f.name for f in Attendance._meta.get_fields()]:
                                kwargs["recorded_by"] = request.user
                            rows.append(Attendance(**kwargs))

                        Attendance.objects.bulk_create(rows, ignore_conflicts=True)

                    # อัปเดตสถิติใน ExamSession (ถ้ามีโมเดลนี้)
                    if ExamSession is not None:
                        session, _ = ExamSession.objects.get_or_create(subject=subject)
                        if session.started_at is None:
                            session.started_at = timezone.make_aware(
                                datetime.combine(subject.exam_date, subject.start_time)
                            )
                        if session.ended_at is None:
                            session.ended_at = timezone.now()
                        if hasattr(session, "update_statistics"):
    ...
    students = subject.students.all().order_by(
        "student_class", "student_number", "user__first_name"
    )
    ...
    attendance_dict = {a.student_id: a for a in attendance_qs}

    # แนบเรคคอร์ดให้ object student + คำนวณสถานะสำหรับแสดงผล
    def compute_calc_status(student, attendance_record):
        if attendance_record:
            return attendance_record.status
        return "absent" if exam_completed else "pending"

    for s in students:
        a = attendance_dict.get(s.id)
        s.attendance_record = a
        # ถ้ามี helper เดิมในโปรเจกต์ ใช้ได้; ไม่งั้นใช้ fallback ด้านล่าง
        try:
            from .views import get_student_status  # ถ้ามีอยู่แล้ว
            s.calculated_status = get_student_status(s, a, subject)
        except Exception:
            s.calculated_status = compute_calc_status(s, a)

    # ===== สถิติ =====
    total = students.count()
    on_time = attendance_qs.filter(status="on_time").count()
    late = attendance_qs.filter(status="late").count()
    excused = attendance_qs.filter(status="excused").count()
    cheating = attendance_qs.filter(status="cheating").count()
    explicit_absent = attendance_qs.filter(status="absent").count()
    # ถ้าหมดเวลาแล้ว: คนที่ไม่มีเรคคอร์ด = ขาดสอบ
    missing = max(0, total - attendance_qs.count())
    absent = explicit_absent + (missing if exam_completed else 0)
    ...
    stats = {
        ...
        "excused_percentage": pct(excused),
        "cheating_percentage": pct(cheating),
    }

    # (ถ้าหน้า template มีส่วนของ teacher_stats อยู่ เดิมใช้อะไรต่อ เติมได้ที่นี่)
    teacher_stats = None

    return render(
        request,
        "app/staff/exam_attendance.html",
        {
            "subject": subject,
            "students": students,
            ...
            "can_edit": can_edit,  # ให้ครูคุมสอบเห็นปุ่ม/เช็คบ็อกซ์เหมือน admin
        },
    )
...
def get_exam_stats(request, subject_id):
    ...
    finalize_absent_students(subject, request.user)
...
def manual_checkin(request):
    ...
    # อนุญาต: ครูหลัก/ครูสำรองของวิชานั้น
    is_teacher = hasattr(request.user, 'teacher_profile')
    is_invigilator = is_teacher and (
        subject.invigilator == request.user.teacher_profile or
        subject.secondary_invigilator == request.user.teacher_profile
    )
    if not (request.user.is_staff or is_invigilator):
    ...
    # --- ทำงานอัปเดตเหมือนเดิม ---
    student = get_object_or_404(StudentProfile, id=student_id)
    attendance, _ = Attendance.objects.get_or_create(student=student, subject=subject)
    attendance.status = status
    attendance.manually_updated = True
...
def finalize_absent_students(subject, performed_by=None) -> int:
    """
    เมื่อการสอบของ subject สิ้นสุด:
    - สร้าง Attendance(status='absent') ให้ทุกคนที่ยังไม่มีเรคอร์ด
    - คืนจำนวนเรคอร์ดที่ถูกสร้าง
    - ปลอดภัยเรียกซ้ำได้ (idempotent)
    """
    now = timezone.now()
    finished = (subject.exam_date < now.date() or
                (subject.exam_date == now.date() and subject.end_time < now.time()))
    if not finished:
        return 0

    # หา student ที่ยังไม่มี Attendance ของวิชานี้
    student_ids = set(subject.students.values_list('id', flat=True))
    existing_ids = set(
        Attendance.objects.filter(subject=subject).values_list('student_id', flat=True)
    )
    missing_ids = student_ids - existing_ids
    if not missing_ids:
        return 0

    to_create = [
        Attendance(subject=subject, student_id=sid, status='absent')
        for sid in missing_ids
    ]
    ...
    # bulk_create ไม่ยิง signal -> อัปเดตสถิติ session เอง
    try:
        session, _ = ExamSession.objects.get_or_create(subject=subject)
        session.update_statistics()
    except Exception:
...
def exam_seating_view(request, pk):
    ...
    return render(request, 'app/staff/exam_seating.html', {
        ...
        'stats': stats,
        'exam_completed': exam_completed,
        'can_edit': can_edit,  # เพิ่มตัวแปรนี้
    })
