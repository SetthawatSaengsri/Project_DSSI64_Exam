# forms.py — เวอร์ชันรวม/แก้ไขแล้ว
...
class UserEditForm(forms.ModelForm):
    ...
    class Meta:
        ...
        fields = [
            ...
            'is_active', 'is_student', 'is_teacher', 'is_staff'
        ]
        widgets = {
...
class ExamSubjectSearchForm(forms.Form):
    ...
    # จะสร้าง ChoiceField แบบไดนามิกใน __init__
                ...
                try:
                    ...
                    if pd.notna(row['exam_date']):
                        if isinstance(row['exam_date'], str):
                            ...
                            # ถ้าเป็น datetime object จาก pandas
...
class QuickExamSetupForm(forms.Form):
    """ฟอร์มตั้งค่าการสอบแบบด่วน - เวอร์ชันปรับปรุงแล้ว (เหลืออันเดียว)"""
...
class ExamRoomForm(forms.ModelForm):
    """ฟอร์มเพิ่ม/แก้ไขห้องสอบ - (ไม่มีฟิลด์โปรเจคเตอร์/แอร์)"""
    class Meta:
        model = ExamRoom
...
class CheatingReportForm(forms.Form):
    """ฟอร์มรายงานทุจริตในการสอบ"""
    ...
    cheating_type = forms.ChoiceField(
        choices=CHEATING_TYPE_CHOICES,
        label="ประเภทการทุจริต",
        widget=forms.Select(attrs={'class': 'form-control'})
    )
    description = forms.CharField(
        ...
        widget=forms.Textarea(attrs={
            'class': 'form-control',
            'rows': 4,
            'placeholder': 'อธิบายรายละเอียดพฤติกรรมทุจริตที่พบ'
        })
    )
    action_taken = forms.CharField(
        ...
        widget=forms.Textarea(attrs={
            'class': 'form-control',
            'rows': 3,
            'placeholder': 'เช่น ตักเตือน, ยึดกระดาษคำตอบ, แจ้งผู้ปกครอง'
        })
    )
    witness = forms.CharField(
        ...
        widget=forms.TextInput(attrs={
            'class': 'form-control',
            'placeholder': 'ชื่อครูหรือเจ้าหน้าที่ที่เป็นพยาน'
        })
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        super().__init__(*args, **kwargs)
        if subject:
            self.fields['student'].queryset = subject.students.all().order_by('student_number')
            self.fields['student'].label_from_instance = (
                lambda obj: f"{obj.student_id} - {obj.user.get_full_name()}"
            )
